...
``` bash
python egomimic/trainHydra.py \
  --config-path=../logs/<name>/<run_dir>/.hydra/ \
  --config-name=config \
  hydra.searchpath=[file://egomimic/hydra_configs] \
  ++mode=eval \
  +evaluator=eval_video \
  +norm_stats.precomputed_norm_path="logs/<name>/<run_dir>/norm_stats/norm_stats.json" \
  ++ckpt_path="'logs/<name>/<run_dir>/checkpoints/last.ckpt'"
```
