...
class CrossAttention(nn.Module):
    ...
    # Intent-Head Co-Train diagnostic: when True, CrossAttention caches the
    # post-softmax attention tensor from the last forward pass onto
    # ``_last_attn``. Set once on the class (via enable_intent_attn_record) and
    # left on for the whole training run — overhead is a single detach per call.
    ...
    def forward(
        ...
        # Cache pre-dropout attention for intent diagnostic. Stored as
        # (B, heads, queries, keys). Only written during training: val-time
        # forward passes would waste ~100 MB of GPU memory on tensors no
        # callback reads.
        if CrossAttention.record_attn and self.training:
            b_h, i, j = attn.shape
            self._last_attn = attn.detach().view(b_h // h, h, i, j)
...
class MultiBlockTransformerDecoder(PolicyHead):
    def __init__(
        ...
        # Optional FiLM modulation injected by HPTModel for the FiLM-gated
        # intent path (docs/intent_head_cotrain_new.md §2.4). Stash-and-consume:
        # HPTModel.configure_action_head_film() pushes (film_module,
        # intent_latent, p_rec) before each head forward; forward applies it
        # once after the last block, then clears. Defaults to no-op so the
        # token-concat baseline (Mode 2 _rw config) is unchanged.
        self._film_module: nn.Module | None = None
        self._film_intent_latent: torch.Tensor | None = None
        self._film_p_rec: torch.Tensor | None = None
    ...
    def set_film(
        self,
        film_module: nn.Module,
        intent_latent: torch.Tensor,
        p_rec: torch.Tensor,
    ) -> None:
        """Stash FiLM args for the next ``forward`` call. Consumed once."""
        self._film_module = film_module
        self._film_intent_latent = intent_latent
        self._film_p_rec = p_rec

    def clear_film(self) -> None:
        self._film_module = None
        self._film_intent_latent = None
        self._film_p_rec = None
    ...
    def forward(self, x):
        ...
        # FiLM modulation (after all decoder blocks, before final_norm /
        # out_proj). No-op if HPTModel didn't push args via set_film().
        if (
            self._film_module is not None
            and self._film_intent_latent is not None
            and self._film_p_rec is not None
        ):
            tokens = self._film_module(
                tokens, self._film_intent_latent, self._film_p_rec
            )
            self.clear_film()
