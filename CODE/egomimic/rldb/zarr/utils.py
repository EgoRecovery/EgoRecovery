...
class DataSchematic(object):
    ...
    def infer_norm_from_dataset(
        ...
        # intent_keys (used by Intent-Head Co-Train) get the same per-embodiment
        # zscore normalization as proprio/action keys. They are separated from
        # proprio_keys only so HPT does not route them through the stem pipeline.
        norm_keys.extend(self.keys_of_type("intent_keys", embodiment))
        if len(norm_keys) == 0:
            logger.warning(
    ...
    def _compute_stats_for_array(X):
        # Intent-Head Co-Train writes NaN at invalid frames (see
        # egomimic/scripts/precompute_gt_intent.py) so norm stats ignore the
        # polluted rows without needing a separate mask. Detect-and-switch so
        # behavior is unchanged for proprio / action arrays (no NaN there).
        if np.isnan(X).any():
            return {
                ...
                "quantile_1": np.nanpercentile(X, 1, axis=0),
                "quantile_99": np.nanpercentile(X, 99, axis=0),
                "quantile_0_01": np.nanpercentile(X, 0.01, axis=0),
                "quantile_99_99": np.nanpercentile(X, 99.99, axis=0),
            }
        return {
    ...
    def normalize_data(self, data, embodiment):
        ...
        for key, tensor in data.items():
            if (
                key in self.keys_of_type("proprio_keys", embodiment)
                or key in self.keys_of_type("action_keys", embodiment)
                or key in self.keys_of_type("intent_keys", embodiment)
            ):
                if (
                ...
                # Intent-Head Co-Train: raw gt_intent contains NaN on invalid
                # frames so norm stats can skip them (np.nanmean/np.nanstd).
                # After normalization, those NaN samples are masked out by
                # gt_intent_valid in L_intent, so we zero them here to keep
                # the downstream tensor finite (autograd-safe even when the
                # mask drops them before reducing).
                if key in self.keys_of_type("intent_keys", embodiment):
                    norm_data[key] = torch.nan_to_num(
                        norm_data[key], nan=0.0, posinf=0.0, neginf=0.0
                    )
            else:
