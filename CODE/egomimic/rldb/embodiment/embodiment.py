...
class Embodiment(ABC):
    ...
    def get_kinematics_solver(cls):
        """Return a kinematics solver with ``fk_pos(jnts_Nx6) -> pos_Nx3`` (base frame).

        Required by ``mode='traj_joints'``. Subclasses whose action space is joint
        angles should override this to return an embodiment-specific FK.
        """
        raise NotImplementedError(
            f"{cls.__name__} does not implement get_kinematics_solver; "
            "cannot use mode='traj_joints'"
        )
    ...
    def viz(
        ...
        image,
        viz_data,
        mode=Literal["traj", "traj_joints", "traj+rotation", "axes", "annotations"],
        intrinsics_key=None,
        **kwargs,
    ):
        ...
        if mode == "traj_joints":
            extrinsics_key = kwargs.pop("extrinsics_key", None)
            if extrinsics_key is None:
                raise ValueError("mode='traj_joints' requires 'extrinsics_key'")
            return _viz_traj_joints(
                image=image,
                actions=viz_data,
                intrinsics_key=intrinsics_key,
                extrinsics_key=extrinsics_key,
                kinematics_solver=cls.get_kinematics_solver(),
                **kwargs,
            )
        ...
        raise ValueError(
            ...
            "('traj', 'traj_joints', 'traj+rotation', 'axes', 'annotations')."
        )

    @classmethod
    def get_keymap(cls, keymap_mode: str, norm_mode: bool = False, annotation_key=None, **kwargs):
        """Returns a dictionary mapping from the raw keys in the dataset to the canonical keys used by the model.

        Extra ``**kwargs`` are forwarded to ``_get_keymap`` so specific
        embodiments can accept optional flags (e.g., ``include_intent_targets``
        for Intent-Head Co-Train) without every call site having to know.
        """
        key_map = cls._get_keymap(keymap_mode, **kwargs)
    ...
    @abstractmethod
    def _get_keymap(cls, keymap_mode: str, **kwargs):
        raise NotImplementedError
