...
class ACT(Algo):
    ...
    def __init__(
        ...
        self.camera_keys = self.data_schematic.keys_of_type(
            "camera_keys", self.embodiment_id
        )
        self.proprio_keys = self.data_schematic.keys_of_type(
            "proprio_keys", self.embodiment_id
        )
        if len(self.data_schematic.keys_of_type("action_keys", self.embodiment_id)) > 1:
            raise ValueError("ACT should only have one action key")
        self.ac_key = self.data_schematic.keys_of_type(
            "action_keys", self.embodiment_id
        )[0]
    ...
    def process_batch_for_training(self, batch):
        ...
        for key, value in batch.items():
            key_name = self.data_schematic.zarr_key_to_keyname(key, self.embodiment_id)
    ...
    def forward_eval_logging(self, batch):
        ...
        for ac_key in self.data_schematic.keys_of_type(
            "action_keys", self.embodiment_id
        ):
    ...
    def visualize_preds(self, predictions, batch):
        ...
        """
        # Use the first camera key as the visualization image
        img_key = self.camera_keys[0]
        ims = (batch[img_key].cpu().numpy().transpose((0, 2, 3, 1)) * 255).astype(
            np.uint8
        )
        preds = predictions[self.ac_key]
        gt = batch[self.ac_key]
        ...
        for b in range(ims.shape[0]):
            if preds.shape[-1] == 7 or preds.shape[-1] == 14:
                ...
                # For action dims that don't match known types, skip drawing
                continue
            ...
            try:
                ...
                # Skip visualization if camera transforms are not compatible
                pass
