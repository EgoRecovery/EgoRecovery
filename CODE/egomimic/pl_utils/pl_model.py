...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for k, v in self.model.log_info(info).items():
            # Per-key log routing for SR diagnostics:
            #   *_sr_pos_correct / *_sr_pos_total / *_sr_neg_correct
            #   *_sr_neg_total            → reduce_fx="sum" so the epoch
            #     value is the total count over all batches × all ranks.
            #     Derive epoch recall = correct / total in wandb UI.
            #   *_sr_recall_pos / *_sr_recall_neg → rank_zero_only to
            #     stop empty-bucket NaNs on rank>0 from polluting the
            #     epoch mean across all ranks. Best-effort live curve;
            #     the count metrics above are the source of truth.
            #   everything else                 → existing behavior.
            if k.endswith("_sr_pos_correct") or k.endswith("_sr_pos_total") or \
               k.endswith("_sr_neg_correct") or k.endswith("_sr_neg_total"):
                self.log(
                    "Train/" + k, v,
                    sync_dist=True, reduce_fx="sum",
                    on_step=False, on_epoch=True,
                )
            elif k.endswith("_sr_recall_pos") or k.endswith("_sr_recall_neg"):
                self.log(
                    "Train/" + k, v,
    ...
    def on_after_backward(self):
        ...
        # Keep a fixed key set on every rank every step so sync_dist all-reduce
        # below never sees a key-set mismatch and hangs DDP.
        info = {
            "policy_grad_norms_raw": grad_norm_val,
            "policy_grad_norms_mad_threshold": float("nan"),
            "policy_grad_norms_mad_flag": 0.0,
        }
