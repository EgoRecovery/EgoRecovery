...
class MultiDataModuleWrapper(LightningDataModule):
    ...
    def _build_combined_loader(self, datasets, params_map, *, shuffle):
        # Under DDP, explicitly use DistributedSampler(drop_last=True) so every rank
        # sees the same number of batches. Without this, Lightning's default wrapper
        # uses drop_last=False and pads unevenly across ranks, which can leave
        # validation/epoch-end barriers waiting indefinitely and trip the NCCL
        # watchdog timeout.
        ddp_on = dist.is_available() and dist.is_initialized()
        iterables = dict()
        for dataset_name, dataset in datasets.items():
            dataset_params = params_map.get(dataset_name)
            if dataset_params is None or len(dataset_params) == 0:
                raise ValueError(
                    f"No dataloader params found for dataset {dataset_name}. Please add {dataset_name} into your data config."
                )
            sampler = (
    ...
    def train_dataloader(self):
        return self._build_combined_loader(
            self.train_datasets, self.train_dataloader_params, shuffle=True
        )

    def val_dataloader(self):
        return self._build_combined_loader(
            self.valid_datasets, self.valid_dataloader_params, shuffle=False
        )
